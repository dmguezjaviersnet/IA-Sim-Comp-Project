...
from parser.ll1_table_builder import build_LL1_table
...
def ll1_parse(G: Grammar, tokens: List[Token]) -> Tuple[bool, Any]: # Parser predictivo no recursivo (LL)
    ...
    if is_ll1: # Si la gramática es LL(1) es que se puede parsear con este algoritmo
        while stack and current_token_index < len(tokens): # Mientras la pila no esté vacía
            ...
            head, current_symbol, prod_str, prod = stack[-1] # Obtengamos el elemento que está en el tope de la pila
            ...
            if current_symbol.identifier == '$':  # Si lo que hay en el tope de la pila es $ (EOF), entonces ya terminé de parsear y después de este token no hay más tokens por analizar
                stack.pop() # Vacíamos la pila, pues solo había un elemente en ella
                current_token_index += 1 # Y nos desplazamos hacia el próximo token, que no va a ser ninguno, pues ya no hay más nada por analizar
            ...
            else:  # Si no es último no-terminal de la producción
                if current_symbol.ast != None: # Y ya se computó el AST del no-terminal actual
                    __eval_rules(head, prod, G.map_prodstr_rules[prod_str][2], False) # Evaluar reglas heredadas
                    stack.pop() # Sacar de la pila el no-terminal
                ...
                elif head != None and len(prod) == 1: # Si el no-terminal cabecera de la producción no es vacio
                    __eval_rules(head, prod, G.map_prodstr_rules[prod_str][2], False) # Evaluar reglas heredadas y continuar a aplicar producciones
